...
#include "ISXEQToolTip.h"

// The mandatory pre-setup function.  Our name is "ISXEQToolTip", and the class is ISXEQToolTip.
...
// our instanced class.
ISXPreSetup("ISXEQToolTip",ISXEQToolTip);
...
unsigned int ISXEQToolTipXML=0;
...
bool ISXEQToolTip::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQToolTip Loaded");
return true;
}

// shutdown sequence
void ISXEQToolTip::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQToolTipXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQToolTipXML);
...
void ISXEQToolTip::ConnectServices()
{
...
void ISXEQToolTip::RegisterCommands()
{
    // add any commands
...
void ISXEQToolTip::RegisterAliases()
{
    // add any aliases
}

void ISXEQToolTip::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQToolTip::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQToolTip::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQToolTip::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQToolTip::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQToolTip::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQToolTipService)
    //        pISInterface->ShutdownService(this,hISXEQToolTipService);
}

void ISXEQToolTip::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQToolTipXML
    ISXEQToolTipXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQToolTipXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQToolTipXML,"General"))
    {
